...
const FeatureCard = memo(({ item, index }) => {
    ...
    const fallbackColors = [
      ...
      'bg-gradient-to-br from-green-500 to-amber-700',
      'bg-gradient-to-br from-indigo-500 to-cyan-700',
    ...
    return (
      ...
      >
        ...
        {/* Light indigo footer with only h3 title and angled arrow */}
        <div className="absolute bottom-0 left-0 right-0 bg-indigo-100 p-4 flex items-center justify-between">
...
const FeaturedSection = ({ 
  ...
  return (
    <section className="w-full bg-white py-16 px-4 relative overflow-hidden">
      ...
      <div className="absolute right-0 bottom-0 w-96 h-96 bg-green-500 opacity-5 rounded-full -mr-48 -mb-48"></div>
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 md:gap-0">
            <p className="text-[24px] md:text-[34px] lg:text-[44px] font-semibold leading-[30px] md:leading-[40px] tracking-[-0.96px] text-[#292929] max-w-4xl">
              {featuredItems.length > 0 ? 'Exclusive Gifts For You' : 'Featured'}
            </p>
          </div>
        </div>
