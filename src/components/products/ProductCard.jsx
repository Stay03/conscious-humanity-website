...
const ProductCard = ({ product, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform duration-300 hover:shadow-lg hover:-translate-y-1">
        ...
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute top-2 right-2">
            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
              type === 'physical' ? 'bg-indigo-100 text-indigo-800' : 'bg-green-100 text-green-800'
            }`}>
          ...
          <div className="absolute top-2 left-2">
            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
              is_online ? 'bg-green-100 text-green-800' : 'bg-green-100 text-green-800'
            }`}>
      ...
      <div className="p-4">
        <Link to={`/product/${slug}`}>
          <h3 className="text-lg font-semibold text-gray-900 mb-2 line-clamp-2 hover:text-green-600 transition-colors">{title}</h3>
        </Link>
          ...
          <button
            ...
            className={`px-3 py-2 rounded-full text-sm font-medium transition-colors flex items-center justify-center min-w-[100px] ${
              isAddingToCart 
                ...
                : 'bg-green-500 text-gray-900 hover:bg-gray-900 hover:text-white'
            }`}
          >
            {isAddingToCart ? (
              <>
