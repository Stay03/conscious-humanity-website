 ...
 * Now with progression and completion states and Vimeo support
 */
const LessonCard = ({ 
  lesson, 
  sectionTitle, 
  ...
  useEffect(() => {
    // Extract thumbnail when component mounts or lesson changes
    if (lesson.video_url) {
      getVideoThumbnail();
    }
  }, [lesson]);
  
  // Extract video thumbnail if video URL exists
    ...
    try {
      ...
      // Handle Vimeo URLs
        ...
        if (match && match[1]) {
          ...
          try {
            // Vimeo oEmbed API to get video information including thumbnail
            const response = await fetch(`https://vimeo.com/api/v2/video/${videoId}.json`);
            if (response.ok) {
              const data = await response.json();
              if (data && data.length > 0) {
                // Use medium-sized thumbnail
                setThumbnailUrl(data[0].thumbnail_medium);
              }
            }
          } catch (error) {
      ...
      // Not a recognized video URL format
  ...
  // Determine video type for badge display
  const getVideoType = () => {
    if (!lesson.video_url) return null;
    
    if (lesson.video_url.includes('youtube')) return 'YouTube';
    if (lesson.video_url.includes('youtu.be')) return 'YouTube';
    if (lesson.video_url.includes('vimeo')) return 'Vimeo';
        ...
        <div className="mt-3 flex items-center space-x-2">
          {lesson.video_url && (
            <span className={`px-2 py-0.5 text-xs rounded-full ${
              isAvailable ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-100 text-gray-500'
            }`}>
              {videoType || 'Video'}
            </span>
          )}
