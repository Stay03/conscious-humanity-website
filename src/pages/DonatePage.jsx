 ...
 * Now with support for URL parameters: project and amount
 */
const DonatePage = () => {
  // Form state
  ...
  useEffect(() => {
    ...
    if (projectParam) {
      // Format project name to be more readable (replace dashes with spaces and capitalize)
      const formattedProject = projectParam
        .split('-')
        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
        .join(' ');
        
      setFormData(prev => ({ ...prev, project: formattedProject }));
    }
    ...
    if (amountParam) {
      ...
      // Check if the amount is one of the predefined amounts
      if (donationAmounts.includes(amountValue)) {
        setFormData(prev => ({ ...prev, amount: amountValue }));
      } else {
        // If not, set it as a custom amount
        setShowCustomAmount(true);
        setFormData(prev => ({ ...prev, customAmount: amountParam }));
      }
    }
  }, []);
        ...
        <div className="mt-12">
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 text-center">
            ...
            {/* <div className="flex items-center">
              ...
              <span className="text-gray-600">Tax-Deductible Receipt</span>
            </div>
            <div className="flex items-center">
              <svg className="w-6 h-6 text-green-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
